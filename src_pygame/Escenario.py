...
class Escenario:
    ...
    def __init__(self, tamCuadr, surface, muros_destruibles=[], muros_indestructibles=[],
    ...
    def dibujar(self,surface):
        for muro in self.muros_destruibles:
            muro.dibujar(surface)
        for muro in self.muros_indestructibles:
            muro.dibujar(surface)
        for bomba in self.bombas:
            bomba.dibujar(surface)
        for powerup in self.powerups:
            powerup.dibujar(surface)
        for enemigo in self.enemigos:
            enemigo.dibujar(surface)
        for explosion in self.explosiones:
            explosion.dibujar(surface)
        self.bomberman.dibujar(surface)
    ...
    def ponerMurosDestruibles(self, cantidad):
        for i in range(cantidad):
            ...
            while True: # Se va a poner sobre un muro indestructible
                if posx%2 == 1 or posy%2 == 1: # Al menos uno es impar
                    break
                if posx%2 == 0:
                    posx = random.randint(1,14)
                else:
